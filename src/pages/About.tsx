...
/* --------------------------------------------------------------- */
/*  ProcessTimeline – restored original staggered animation        */
/* --------------------------------------------------------------- */
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      (entries) => {
        if (entries[0].isIntersecting) {
          steps.forEach((_, i) => {
            setTimeout(() => setActiveStep(i), i * 300);
          });
        }
      },
      { threshold: 0.3 }
            ...
            return (
              ...
              >
                ...
                <div className="flex items-start gap-6 md:hidden">
                  <div className="relative flex-shrink-0">
                    <div
                      className={`w-16 h-16 rounded-full bg-gradient-to-br from-primary to-accent flex items-center justify-center transition-transform duration-500 ${
                        isActive ? "scale-100" : "scale-0"
                      }`}
                    >
                      <IconComponent className="w-8 h-8 text-white" />
                ...
                <div className="hidden md:flex items-center w-full">
                  <div
                    className={`flex-1 text-right pr-12 ${
                      index % 2 === 0 ? "" : "order-2 text-left pl-12 pr-0"
                    }`}
                  >
                    <div className="text-3xl font-bold text-muted-foreground/30 mb-2">
                      {String(index + 1).padStart(2, "0")}
...
const About = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      setLoading(true);
      try {
        const [aboutRes, cardsRes] = await Promise.all([
          fetchAbout(),
          fetchCards(),
        ]);
        if (aboutRes?.success) setAboutData(aboutRes.data);
  ...
  /* ----- Timeline steps ----- */
  const processCard = cards.find(
    (c: any) => c.type === "about" && c.features?.length === 6
  );
  const timelineSteps =
    processCard?.features.map((feature: string, i: number) => ({
      title: feature,
      description:
        processCard.description || `Step ${i + 1} of our process`,
    })) || [
      {
        title: "Listen",
        description:
          "We map your processes, constraints, and success criteria - no assumptions.",
      },
      {
        title: "Design",
        description:
          "Clear models, intuitive flows, and a delivery plan you can rely on.",
      },
      {
        title: "Build",
        description:
          "Incremental releases with testing, reviews, and enterprise-grade tooling.",
      },
      {
        title: "Support",
        description:
          "Maintenance, upgrades, and improvements that deliver continuous value.",
      },
    ];

  const aboutCards = cards.filter(
    (c: any) =>
      c.type === "about" && c.isActive && c._id !== processCard?._id
  );
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="pt-32 pb-24 px-6">
        <div className="container mx-auto max-w-6xl">
          ...
          {/* CTA – button now has solid background */}
          <div className="mt-24 text-center">
            ...
            <a
              href={aboutData.ctaButtonLink}
              className="inline-flex items-center justify-center rounded-lg px-8 py-4 bg-primary hover:bg-primary/90 text-primary-foreground font-semibold transition-colors"
            >
              {aboutData.ctaButtonText}
            </a>
          </div>
